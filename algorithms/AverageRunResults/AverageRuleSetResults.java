...
public class AverageRuleSetResults {
	...
	private static void readInput(String inputFileName) throws FileNotFoundException, IOException {
		...
		ruleSetConditionCounts = new double[NUM_SIMS];
		ruleCounts = new double[NUM_SIMS];
		averageConditionsPerRuleCounts = new double[NUM_SIMS];
	...
	// ===================================================================================================
	// Method  : void storeRuleSetConditionCount(String, StringTokenizer, int)
	// Purpose : Stores the rule set condition counts in the appropriate array
	// ===================================================================================================
	
	private static void storeRuleSetConditionCount(String inputFileName, StringTokenizer tokenizer, int recordNumber) {
		
		if (tokenizer.countTokens() == 0) {
			System.err.println("Error finding rule set condition count, record " + (recordNumber + 1) + ", file " + inputFileName);
			ruleSetConditionCounts[recordNumber] = Double.MAX_VALUE;
			return;
		}
		...
		try {
			ruleSetConditionCounts[recordNumber] = Double.parseDouble(token);
		} catch (NumberFormatException e) {
	...
	// ===================================================================================================
	// Method  : void storeAverageConditionsPerRuleCount(String, StringTokenizer, int)
	// Purpose : Stores the average conditions per rule counts in the appropriate array
	// ===================================================================================================
	
	private static void storeAverageConditionsPerRuleCount(String inputFileName, StringTokenizer tokenizer, int recordNumber) {
		
		if (tokenizer.countTokens() == 0) {
			System.err.println("Error finding average conditions per rule count, record " + (recordNumber + 1) + ", file " + inputFileName);
			averageConditionsPerRuleCounts[recordNumber] = Double.MAX_VALUE;
			return;
		}
		...
		try {
			averageConditionsPerRuleCounts[recordNumber] = Double.parseDouble(token);
		} catch (NumberFormatException e) {
	...
	private static void storeInput(String inputFileName, String readLine, int recordNumber) {
		...
		if (readLine.equals("")) {
			...
			if (tokenizer.countTokens() > 9) {
				System.err.print((tokenizer.countTokens() - 9) + " extra values found, record " + (recordNumber + 1));
		...
		storeRuleSetConditionCount(inputFileName, tokenizer, recordNumber);
		storeRuleCount(inputFileName, tokenizer, recordNumber);
		storeAverageConditionsPerRuleCount(inputFileName, tokenizer, recordNumber);
	...
	private static void calcMeasures() {
		...
		for (int i = 0; i < NUM_SIMS; i++) {
			...
			meanRuleSetConditionCount += ruleSetConditionCounts[i];
			meanRuleCount += ruleCounts[i];
			meanAverageConditionsPerRuleCount += averageConditionsPerRuleCounts[i];
		}
		...
		meanRuleSetConditionCount /= (double) NUM_SIMS;
		meanRuleCount /= (double) NUM_SIMS;
		meanAverageConditionsPerRuleCount /= (double) NUM_SIMS;
		...
		stdDevRuleSetConditionCount = 0.0;
		stdDevRuleCount = 0.0;
		stdDevAverageConditionsPerRuleCount = 0.0;
		...
		for (int i = 0; i < NUM_SIMS; i++) {
			...
			stdDevRuleSetConditionCount += Math.pow(ruleSetConditionCounts[i] - meanRuleSetConditionCount, 2.0);
			stdDevRuleCount += Math.pow(ruleCounts[i] - meanRuleCount, 2.0);
			stdDevAverageConditionsPerRuleCount += Math.pow(averageConditionsPerRuleCounts[i] - meanAverageConditionsPerRuleCount, 2.0);
		}
		...
		stdDevRuleSetConditionCount = Math.sqrt(stdDevRuleSetConditionCount / (((double) NUM_SIMS) - 1.0));
		stdDevRuleCount = Math.sqrt(stdDevRuleCount / (((double) NUM_SIMS) - 1.0));
		stdDevAverageConditionsPerRuleCount = Math.sqrt(stdDevAverageConditionsPerRuleCount / (((double) NUM_SIMS) - 1.0));
	...
	private static double[] ruleSetConditionCounts;
	private static double[] ruleCounts;
	private static double[] averageConditionsPerRuleCounts;
	...
	private static double meanRuleSetConditionCount;
	private static double meanRuleCount;
	private static double meanAverageConditionsPerRuleCount;
	...
	private static double stdDevRuleSetConditionCount;
	private static double stdDevRuleCount;
	private static double stdDevAverageConditionsPerRuleCount;
