...
// ===================================================
// Utility : ostream& outputClusterVector(ostream&) const
// Purpose : Prints current cluster vector to ostream
// ===================================================

std::ostream& SOMMap::outputClusterVector(std::ostream& out) const {
	
	for (unsigned int i = 0; i < clusterVector.size(); ++i) {
		out << i << ": " << *(clusterVector[i]);
		
		if (i != (clusterVector.size() - 1)) {
			out << std::endl;
		}
	}
...
// ===================================================
// Utility : ostream& outputClusterCosts(ostream&) const
// Purpose : Prints current cluster costs to ostream
// ===================================================

std::ostream& SOMMap::outputClusterCosts(std::ostream& out) const {
	
	int clusterCount;
	
	if (clusterVector.size() > 1) {
		clusterCount = clusterVector.size();
	} else {
		clusterCount = 2;
	}
	
	for (unsigned int i = 0; i < clusterCosts.size(); ++i, ++ clusterCount) {
		
		out << clusterCount << " clusters: " << clusterCosts[i];
		
		if (i != (clusterCosts.size() - 1)) {
			out << std::endl;
		}
	}
