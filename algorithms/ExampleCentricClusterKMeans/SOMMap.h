...
class SOMMap {
	...
	public:
		...
		// utility functions
		std::vector<Centroid> optimalKMeansClusterMap();
		void kMeansLabelMap(std::vector<Centroid>, DataFileReader&);
	...
	protected:
		// accessors
		std::vector<Centroid> kMeansCluster(unsigned int) const;
	...
	private:
		...
		double calculateDaviesBouldinIndex(std::vector<Centroid>) const throw (std::runtime_error);
