...
int main(int argc, char** argv) {
	...
	if (argc != 7) {
		...
		std::cerr << "Usage: classify [SOMFileName] [trainingDataFileName] [testDataFileName] [NamingFileName] [accuracyOutFile] [errorOutFile]" << std::endl;
		return 1;
	}
	...
	try {
		...
		std::vector<Centroid> optimalCentroids = readMap->optimalKMeansClusterMap();
		std::cout << " Done" << std::endl;
		
		// label map
		std::cout << "Labeling map vectors";
		readMap->kMeansLabelMap(optimalCentroids, trainingDataReader);
		std::cout << " (using " << optimalCentroids.size() << " clusters)... Done" << std::endl;
		...
		outputAccuracyWriter << readMap->getTrainingAccuracy() << "\t" << "0.000000000000000"                      << "\t" << "0.000000000000000"                     << "\t" << readMap->getTestAccuracy() << "\t" << "0.000000000000000"                  << "\t" << "0.000000000000000"                 << "\t" << readMap->getUnlabeledWeights() << "\t" << readMap->getUnlabeledWeightPercentage() << std::endl;
