...
MapCluster(_clusteringLevel), subCluster1(&_subCluster1), subCluster2(&_subCluster2) {
	...
	for (unsigned int i = 0; i < cluster1Centroid.size(); ++i) {
		...
		double newCentroidComponent = ((cluster1Count * cluster1Centroid[i]) + (cluster2Count * cluster2Centroid[i])) / (cluster1Count + cluster2Count);
		
		if (!std::isfinite(newCentroidComponent)) {
			std::cerr << std::endl << "ERROR: Invalid centroid component computed" << std::endl;
		}
		
		centroid.push_back(newCentroidComponent);
	}
...
// ===================================================
// Accessor : double getCentroidWeight(unsigned int) const
// Purpose  : Returns the specified weight from this
//            cluster's centroid
// ===================================================

double AggregateMapCluster::getCentroidWeight(unsigned int weightIndex) const {
	
	return centroid[weightIndex];
...
double AggregateMapCluster::getIntraClusterDistance() const {
	...
	double returnValue = (intraClusterDistance / ((double) constituentWeightVectors.size()));
	
	if (!std::isfinite(returnValue)) {
		std::cerr << std::endl << "ERROR: Invalid intra cluster distance computed" << std::endl;
	}
