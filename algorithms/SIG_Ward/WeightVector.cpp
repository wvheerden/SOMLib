...
// ===================================================
// Accessor : double getWeight(unsigned int) const
// Purpose  : Returns specified constituent weight
// ===================================================

double WeightVector::getWeight(unsigned int index) const {
...
double WeightVector::getCentroidWeight(unsigned int weightIndex) const {
...
double WeightVector::getEuclideanDistance(const std::vector<double> vec) const throw (std::runtime_error) {
	...
	double returnValue = sqrt(returnDistance);
	
	if (!std::isfinite(returnValue)) {
		std::cerr << std::endl << "ERROR: Invalid Euclidean distance computed" << std::endl;
	}
