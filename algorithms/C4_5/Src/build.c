...
{ 
    ...
    /*  Test for minimum number of nodes is an optimisation, because it means that no test can possibly satisfy
        the requirement for MINOBJS examples under two nodes, which will in turn mean that all test will have
        zero gain, and therefore zero gain ratio, producing a leaf node  */
	...
	ForEach(v, 1, Node->Forks)
	{
	    ...
	    {
		/* Create a leaf node if there are no examples satisfying outcome v (?) */
