    ...
    ClassNo TargetClass;
{
	...
	ForEach(d, 0, NCond)
	{
		...
		/*  Also when actual error rate for pruned rule is lower than
		    actual error rate for unpruned rule (however, this is not
		    mentioned in any of the papers, and may be an impossible
		    condition, because it is subsumed by the pessimistic
		    error rate)  */
	...
	if ( Alter )
	{
	    ...
	    /*  We detect a deletable condition either according to the
	        pessimistic error rate, or the Fisher significance test,
	        however, we always delete the condition with the lowest
	        pessimistic error rate if one can be deleted.  */
