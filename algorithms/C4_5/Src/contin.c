...
{ 
    ...
    /*  ThreshCost is the MDL-based "charge" to the gain discussed in Quinlan:1996a  */
    /*  Val is the adjusted gain discussed in Quinlan:1996a  */
    /*  Note that adjusted gain is always used to find the best threshold, regardless of
        whether gain or gain ratio is used in the final evaluation of the tests  */
    ...
    if ( BestI == None )
    {
	/*  Gain (and therefore gain ratio) is set to 0 if adjusted gain is <= 0 for all candidate splits on this attribute  */
	Gain[Att] = -Epsilon;
	Info[Att] = 0.0;
    ...
    {
	/*  Gain is set to adjusted gain, meaning that gain ratio for continuous attributes is also calculated using adjusted gain  */
