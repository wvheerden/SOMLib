...
Boolean	*RuleIn,	/* RuleIn[r]	= true if rule r included */
	...
	**Match;	/* Match[r][i]	= true if rule r fires on item i */
                        /* Seemingly a rule only fires for a data example if the condition matches and the pessimistic accuracy rate is > 0.1 */
    ...
    RuleNo RuleCount;
{
	...
	/*  Perform a local greedy search to add and remove rules from the random subset  */
	HillClimb(RuleCount);
    }

    /*  At this point, Subset contains the subset of rules for the group with the lowest
        MDL measure over all trials, where each trial has randomly added rules to a subset
        and then performed a local greedy search on the subset  */
}
    ...
    RuleNo RuleCount;
{
    ...
    while ( true )
    {
	/*  Update Subset so that it contains the lowest MDL value found thus far  */
	CalculateValue();
	...
	{
	    r = ClassRules[ri];
	    /*  Delta is related to the advantage attributable to rule r, or realisable if rule r included, according to the MDL encoding  */
	    Delta = ( RuleIn[r] ? -Value[r] : Value[r] );
	...
	/*  We stop adding and deleting rules when we can't find a Delta that is not zero  */
	if ( ! Bestr ) break;

	/*  If we've found a highest Delta that is not zero, we either delete (if the rule
	    is in the subset) or add (if the rule is not in the subset) the rule in question  */
	if ( RuleIn[Bestr] )
	{
	    DeleteRule(Bestr);
	}
    ...
    /*  At this point, Subset contains the best subset of tests for the group, according to the MDL measure,
        after the local greedy search has added and removed all the rules it can  */
}
...
/*  This uses the MDL theory computations from Quinlan:1995  */
...
/*  --------------  */
{
    ...
    /*  BaseBits is the MDL encoding value  */
    ...
    /*  For every rule, store the advantage attributable to the rule, or realisable if the rule is included  */
    /*  The Value array stores the delta values mentioned in Quinlan:1993, that are used for simulated annealing and the local  greedy search  */
    ForEach(r, 1, NRules)
    {
	if ( Rule[r].Rhs == FocusClass )
	{
    ...
    /*  Store the currently found best subset overall (the one with the lowest MDL
        encoding value over all subsets that have been investigated so far, by
        whatever search process is followed) in Subset, with its MDL encoding value
        in SubsetValue  */
