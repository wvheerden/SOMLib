...
char *usage[] = {
	...
	"  -W double          white treshold\n",
	"  -B double          black treshold\n",
	...
	"  -fontsize double   fontsize relative to the radius of an unit\n",
...
double white_treshold = 1.0;
double black_treshold = 0.0;
...
main (int argc, char **argv)
{
	...
	fontsize =
		oatof (extract_parameter (argc, argv, "-fontsize", OPTION), -1.0);
	...
	if (extract_parameter (argc, argv, "-swapx", OPTION2))
		swapx = 1;
	if (extract_parameter (argc, argv, "-swapy", OPTION2))
		swapy = 1;
...
int
change_suffix (char *s, char *dest, char *end)
{
	char *s1;
	strcpy (dest, s);
...
int
print_page (FILE * fp, struct eps_info *einfo)
{
	...
	scale1 = (double) pw / (double) w;
	scale2 = (double) ph / (double) h;
	...
	if ((title_str) && (!notitle)) {
		...
		fprintf (fp, "0 setgray %f %f 8 add moveto\n",
						 (double) 2.0, einfo->height);
		fprintf (fp, "(%s - Dim: %d, Size: %d*%d units, %s neighborhood) show\n",
						 ps_string_filter (title_str), umat->dim, umat->mxdim,
						 umat->mydim,
...
int
print_header (FILE * fp, char *hname)
{
	int c;
	struct file_info *fi;
	FILE *header_file;
...
int
image_size (struct eps_info *einfo, int width)
{
	struct umatrix *umat = einfo->umat;
	...
	switch (umat->topol) {
	case TOPOL_RECT:
		einfo->xstep = (double) width / (double) umat->uxdim;
		...
		break;
	case TOPOL_HEXA:
		einfo->xstep = (double) width / (double) (umat->uxdim + 1);
...
int
print_eps (FILE * fp, struct eps_info *einfo)
{
	double color;
	double xstep, ystep, radius;
	int x, y, numlabs, i;
	char *label;
	eptr p;
	...
	/*  radius = eps_width / (2.0 * sqrt(3) * (double)umat->mxdim); */
	xstep = einfo->xstep;
	ystep = einfo->ystep;
	radius = einfo->radius;
	...
	fprintf (fp, "/radius %f def\n/xstep %f def\n/ystep %f def\n",
					 radius, xstep, ystep);
	fprintf (fp, "/picwidth %f def /picheight %f def\n",
	...
	fprintf (fp, "/xoffset %f def /yoffset %f def\n",
	...
	if (drawblocks)
		for (y = 0; y < umat->uydim; y++) {
			...
			for (x = 0; x < umat->uxdim; x++)
				fprintf (fp, "%d %s ", (int) (100 * umat->uvalue[x][y]),
								 draw_block_s);
			fprintf (fp, "%s\n", end_row_s);
		}
