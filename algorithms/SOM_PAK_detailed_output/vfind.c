...
main (int argc, char **argv)
{
	...
	int xdim, ydim, error;
	int topol, neigh;
	double alpha1, radius1;
	int fixed, weights;
	double alpha2, radius2;
	double qerror, qerrorb;
	...
	long buffer, length1, length2, noc, nod = 0;
	int qmode;
	//struct typelist *type_tmp;  removed due to adaptive stopping conditions
	char *funcname = NULL;
	...
	alpha1 = get_double ("Give the training rate of first part", 0.0);
	radius1 = get_double ("Give the radius in first part", 0.0);
	length2 = get_int ("Give the training length of second part", 0);
	alpha2 = get_double ("Give the training rate of second part", 0.0);
	radius2 = get_double ("Give the radius in second part", 0.0);
  ...
  */
	...
	while (not) {
		...
		set_teach_params (&params, codes, NULL, NULL, NULL, 0, funcname);
		set_som_params (&params);
		params.data_training = data;
		...
		params.data_validation_error = testdata;
		ifverbose (2)
			fprintf (stderr, "Calculating quantization error\n");

    // find quantisation error on DATA SET
		if (qmode > 0)
			qerror = find_validation_qerror2 (&params);
		else
			qerror = find_validation_qerror (&params);
		...
		codes = NULL;
		ifverbose (1)
			fprintf (stderr, "%3ld: %f\n", not, qerror / (double) nod);
		not--;
	}
	...
	if (codess != NULL) {
		...
		ifverbose (1)
			fprintf (stdout, "Smallest error with random seed %3ld: %f\n",
							 bnot, qerrorb / (double) nod);
	}
