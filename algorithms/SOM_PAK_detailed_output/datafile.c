...
struct entries *read_entries(struct entries *entries)
{
  ...
  // the variable freed is also never used (Lord knows why)
  {
    long freed = 0;
    while (next) {
      entr = next->next;
						 ...
						 struct data_entry *entry)
{
	...
	for (i = 0; i < entr->dimension; i++)
		if ((entry->mask != NULL) && (entry->mask[i] != 0))
			...
			fprintf (fp, "%.*g ", DBL_DIG, entry->points[i]);     //here
	...
	/* Some kind of error checking could be added ... */
  // ... but I have better things to keep my time occupied with - like smoking crack.
	return 0;
}
...
// which I have now removed because it is crap, confusing, and
// never gets called under any reaspoable circumstances
...
*/
  ...
  // modified to co-incide with Java's shuffle implementation
  for (i = nol - 1; i >= 1; i--) {
    j = orand(i + 1);
    temp = tbl[i];
    tbl[i] = tbl[j];
...
// one has to wonder what the programmer was smoking at the time he/she
// came up with this magnificent gem
#if 0
