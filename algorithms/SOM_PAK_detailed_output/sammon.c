...
struct entries *
remove_identicals (struct entries *codes, int *rem)
{
...
struct entries *
sammon_iterate (struct entries *codes, int length)
{
	long i, j, k;
	long noc = 0;
	...
	x = (double *) oalloc (sizeof (double) * noc);
	y = (double *) oalloc (sizeof (double) * noc);
	xu = (double *) oalloc (sizeof (double) * noc);
	yu = (double *) oalloc (sizeof (double) * noc);
	dd = (double *) oalloc (sizeof (double) * (noc * (noc - 1) / 2));

	/* Initialize the tables */
	for (i = 0; i < noc; i++) {
    //x[i] = (double) (orand () % noc) / noc;
		x[i] = (double) (orand (noc)) / noc;
		y[i] = (double) (i) / noc;
	}
	...
	for (i = 0; i < length; i++) {
		for (j = 0; j < noc; j++) {
			...
			for (k = 0; k < noc; k++) {
				...
				xd = x[j] - x[k];
				yd = y[j] - y[k];
				dpj = (double) sqrt ((double) xd * xd + yd * yd);
		...
		for (j = 1; j < noc; j++)
			for (k = 0; k < j; k++) {
				...
				xd = x[j] - x[k];
				yd = y[j] - y[k];
				ee = d - (double) sqrt ((double) xd * xd + yd * yd);
				e += (ee * ee / d);
				mutual++;
			}
		e /= tot;
	...
	prev = &tmp;
	prev->next = NULL;
	for (i = 0, entr = rewind_entries (codes, &p1); i < noc;
			 i++, entr = next_entry (&p1)) {
...
void
save_entries_in_eps (struct entries *spics, char *filename, int ps, int rem)
{
	char str[100];
	FILE *fp;
...
main (int argc, char **argv)
{
	...
	length = oatoi (extract_parameter (argc, argv, RUNNING_LENGTH, ALWAYS), 1);
	randomize = oatoi (extract_parameter (argc, argv, RANDOM, OPTION), 0);
