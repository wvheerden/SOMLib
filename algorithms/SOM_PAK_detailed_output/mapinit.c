...
{
	long number_of_codes, randomize, buffer;
	...
	xdim = oatoi (extract_parameter (argc, argv, XDIM, ALWAYS), 0);
	ydim = oatoi (extract_parameter (argc, argv, YDIM, ALWAYS), 0);
...
// HERE
	...
	sprintf (comments, "# random seed: %ld\n", randomize);
