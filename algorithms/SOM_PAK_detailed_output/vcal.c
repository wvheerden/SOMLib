...
struct entries *
find_labels (struct teach_params *teach, int numlabs)
{
	long noc, nol, index, i;
	int labs;
	int datalabel, ind;
	struct data_entry *codetmp, *datatmp;
	WINNER_FUNCTION *winner = teach->winner;
	struct entries *data = teach->data_training;
	...
	for (i = 0; i < noc; i++) {
		hits[i] = new_hitlist ();
		if (hits[i] == NULL) {
			fprintf (stderr, "Can't get hitlist[%ld]\n", i);
