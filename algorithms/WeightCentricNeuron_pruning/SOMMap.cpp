...
width(_width), height(_height), weightDimension(_weightDimension), mapGrid(new WeightVector**[_height]), trainingMisclassificationPercentage(0.0), testMisclassificationPercentage(0.0), prunedNeuronCount(0) {
...
// ===================================================
// Utility : void weightCentricLabelMap(DataFileReader&)
// Purpose : Initiates neuron labelling process using
//           the weight vector centric method
// ===================================================

void SOMMap::weightCentricLabelMap(DataFileReader& dataReader) {
	
	// consider (and label) all weight vectors in the map
	for (unsigned int i = 0; i < height; ++i) {
		for (unsigned int j = 0; j < width; j++) {
