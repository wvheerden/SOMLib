...
struct file_info *
open_file (char *name, char *fmode)
{
	FILE *fp;
	...
	if (name == NULL) {
	...
	} else if (compress || piped_com) {
#ifndef NO_PIPED_COMMANDS
		if (compress) {
			/* compressed files */
			sprintf (buf, (mode == FM_READ) ?
							 uncompress_command : compress_command, name);
		...
		fprintf (stderr,
						 "open_file: file %s: piped commands are not supported is this version\n",
						 name);
		close_file (fi);
		return NULL;
#endif /* !NO_PIPED_COMMANDS */
		...
		if (fp == NULL) {
			fprintf (stderr,
							 ...
							 name, (mode == FM_READ) ? "reading" : "writing", fmode);
			perror ("file_open");
			close_file (fi);
			return NULL;
		}
...
int
close_file (struct file_info *fi)
{
	int retcode = 0;
...
static char *
check_for_compression (char *name)
{
	char *s;
...
/* getln - get a line from file. Returns a char * to the line (a
   static buffer), NULL on error. */

char *
getln (struct file_info *fi)
{
	...
	while (1) {
		...
		if (len >= strl) {
			...
			if (strl > ABS_STR_LNG) {
				fprintf (stderr, "getln: Too long lines in file %s (max %d)\n",
								 fi->name, ABS_STR_LNG);
		...
		if (c == EOF) {
			...
			if (feof (fp)) {
				...
				fprintf (stderr, "getln: read error on line %ld of file %s\n",
								 fi->lineno, fi->name);
				perror ("getln");
			}
			break;
...
int
rewind_file (struct file_info *fi)
{
	char buf[512];
