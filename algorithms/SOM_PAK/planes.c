...
print_plane (int plane)
{
	...
	while (codetmp != NULL) {
		...
		if ((maxval - minval) != 0.0)
			cv = 0.05 + 0.9 * (codetmp->points[plane] - minval) / (maxval - minval);
		else
			cv = 0.5;
		xp = XSTEP * (index % codes->xdim) + XSTEP / 2;
		yp = YSTEP * (index / codes->xdim) + YSTEP / 2;
		if ((index / codes->xdim) % 2)
			xp += offset;

		/* Print the value */
		fprintf (fp, "%d %d %f LN\n", xp, yp, cv);
		fprintf (fp, "%d %d 0.000000 LM\n", xp, yp);
...
void
scan_data_traj (struct entries *codes, struct entries *data)
{
	struct teach_params *teach = &params;
...
print_trajectory ()
{
	...
	if (data_training != NULL) {
		...
		if (traj_ready == 0)
			scan_data_traj (codes, data_training);
...
main (int argc, char **argv)
{
	...
	if (plane > codes->dimension) {
		fprintf (stderr,
						 "Required plane is bigger than codebook vector dimension");
		error = 1;
		goto cleanup;
	}

	set_teach_params (&params, codes, data, NULL, NULL, buffer, funcname);
