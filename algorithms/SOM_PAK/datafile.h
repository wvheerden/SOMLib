...
struct entries *read_entries_no_shuffle1 (struct entries *entries);

int save_entries_wcomments (struct entries *codes, char *out_code_file,
														char *comments);
...
struct data_entry *init_entry (struct entries *entr,
															 struct data_entry *entry);
#define alloc_entry(entr) init_entry((entr), NULL)
void free_entry (struct data_entry *entry);
struct data_entry *load_entry (struct entries *entr,
															 struct data_entry *entry);
...
struct data_entry *copy_entry (struct entries *entries,
...
int alpha_read (double *alphas, long noc, char *infile);
int alpha_write (double *alphas, long noc, char *outfile);
