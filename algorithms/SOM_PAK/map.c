...
struct umatrix *
read_map (char *mapfile, int xswap, int yswap)
{
	int i, j;
	...
	umat->topol = codes->topol;

	umat->uxdim = 2 * umat->mxdim - 1;
	umat->uydim = 2 * umat->mydim - 1;

	/* allocate memory for the needed structures */
	umat->mvalue = (double ***) malloc (umat->mxdim * sizeof (double **));
	for (i = 0; i < umat->mxdim; i++) {
		umat->mvalue[i] = (double **) malloc (umat->mydim * sizeof (double *));
	}

	/* allocate memory for the u-matrix structure */
	umat->uvalue = (double **) malloc (umat->uxdim * sizeof (double *));
	for (i = 0; i < umat->uxdim; i++)
		umat->uvalue[i] = (double *) malloc (umat->uydim * sizeof (double));
...
int
compar (const void *first, const void *sec)
{
	if (*(double *) first < *(double *) sec)
		return -1;
	else
...
int
calc_umatrix (struct umatrix *umat, int xswap, int yswap)
{
	int i, j, k, bx, by, bz;
	...
	if (umat->topol == TOPOL_RECT) {
		for (j = 0; j < umat->mydim; j++)
			for (i = 0; i < umat->mxdim; i++) {
				...
				dz =
					(sqrt (dz1) / sqrt ((double) 2.0) +
					 sqrt (dz2) / sqrt ((double) 2.0)) / 2;
		...
		/* hexagonal topology */
	{
		for (j = 0; j < umat->mydim; j++)
			for (i = 0; i < umat->mxdim; i++) {
				...
				temp = 0;
				if (i < (umat->mxdim - 1)) {	// columns before last column
					for (k = 0; k < umat->dim; k++) {
						temp = (umat->mvalue[i][j][k] - umat->mvalue[i + 1][j][k]);
						dx += temp * temp;
						bx = 1;
				...
				temp = 0;
				if (j < (umat->mydim - 1)) {			// rows before last row
					if (j % 2) {				// even rows (1, 3, 5, ...)
						for (k = 0; k < umat->dim; k++) {
							temp = (umat->mvalue[i][j][k] - umat->mvalue[i][j + 1][k]);
							dy += temp * temp;
							by = 1;
						...
						if (i > 0) {			// columns after first column
							for (k = 0; k < umat->dim; k++) {
								temp = (umat->mvalue[i][j][k] - umat->mvalue[i - 1][j + 1][k]);
								dy += temp * temp;
								by = 1;
				...
				temp = 0;
				if (j < (umat->mydim - 1)) {			// rows before last row
					if (!(j % 2)) {				// odd rows (0, 2, 4, ...)
						for (k = 0; k < umat->dim; k++) {
							temp = (umat->mvalue[i][j][k] - umat->mvalue[i][j + 1][k]);
							dz += temp * temp;
						}
						bz = 1;
					} else {				// even rows (1, 3, 5, ...)
		...
		for (j = 0; j < umat->uydim; j += 2)
			for (i = 0; i < umat->uxdim; i += 2)
				if (i > 0 && j > 0 && i < umat->uxdim - 1 && j < umat->uydim - 1) {
					...
					umat->uvalue[i][j] =
						(umat->uvalue[i + 1][j] + umat->uvalue[i][j + 1]) / 2.0;
...
void
swap_umat (struct umatrix *umat, int x, int y)
{
	int i, j;
	if (x) {
		double tmp;
...
int
average_umatrix (struct umatrix *umat)
{
	int i, j;
	...
	umat2->uvalue = (double **) malloc (umat->uxdim * sizeof (double *));
	for (i = 0; i < umat->uxdim; i++)
		umat2->uvalue[i] = (double *) malloc (umat->uydim * sizeof (double));
	...
	if (umat->topol == TOPOL_RECT) {
		...
		/*corners */
		umat2->uvalue[0][umat->uydim - 1] =
			(umat->uvalue[1][umat->uydim - 1] + umat->uvalue[0][umat->uydim - 1] +
			 umat->uvalue[0][umat->uydim - 2]) / 3.0;
		umat2->uvalue[umat->uxdim - 1][umat->uydim - 1] =
			(umat->uvalue[umat->uxdim - 2][umat->uydim - 1] +
			 umat->uvalue[umat->uxdim - 1][umat->uydim - 1] +
			 umat->uvalue[umat->uxdim - 1][umat->uydim - 2]) / 3.0;
		umat2->uvalue[umat->uxdim - 1][0] =
			(umat->uvalue[umat->uxdim - 2][0] + umat->uvalue[umat->uxdim - 1][0] +
			 umat->uvalue[umat->uxdim - 1][1]) / 3.0;
		umat2->uvalue[0][0] =
			(umat->uvalue[1][0] + umat->uvalue[0][1] + umat->uvalue[0][0]) / 3.0;
		...
		/* hexagonal topology */
	{
				...
				/* Non-borders */
			{
				if ((j % 4) == 1) {
					umat2->uvalue[i][j] = ((umat->uvalue[i][j - 1] +
																	umat->uvalue[i + 1][j - 1] +
																	umat->uvalue[i - 1][j] +
																	umat->uvalue[i][j] + umat->uvalue[i +
																																		1][j] +
																	umat->uvalue[i - 1][j + 1] +
																	umat->uvalue[i][j + 1]) / ((double) 7.0));
				} else if ((j % 4) == 2) {
					umat2->uvalue[i][j] = ((umat->uvalue[i][j - 1] +
																	umat->uvalue[i + 1][j - 1] +
																	umat->uvalue[i - 1][j] +
																	umat->uvalue[i][j] + umat->uvalue[i +
																																		1][j] +
																	umat->uvalue[i][j + 1] + umat->uvalue[i +
																																				1][j +
																																					 1])
																 / ((double) 7.0));
		...
		for (i = 1; i < umat->uxdim - 1; i++)
			umat2->uvalue[i][j] = ((umat->uvalue[i - 1][j] +
															...
															umat->uvalue[i - 1][j + 1] +
															umat->uvalue[i][j + 1]) / ((double) 5.0));
		/*south border */
		j = umat->uydim - 1;
		for (i = 1; i < umat->uxdim - 1; i++) {
			if ((j % 4) == 1) {
				umat2->uvalue[i][j] = ((umat->uvalue[i][j - 1] +
																umat->uvalue[i + 1][j - 1] + umat->uvalue[i -
																																					1]
																[j] + umat->uvalue[i][j] + umat->uvalue[i +
																																				1][j])
															 / ((double) 5.0));
			} else if ((j % 4) == 2) {
				umat2->uvalue[i][j] = ((umat->uvalue[i][j - 1] +
		...
		for (j = 1; j < umat->uydim - 1; j++) {
			if ((j % 4) == 1) {
				...
				umat2->uvalue[i][j] = ((umat->uvalue[i][j - 1] +
																umat->uvalue[i - 1][j] +
																umat->uvalue[i][j] +
																umat->uvalue[i][j + 1]) / ((double) 4.0));

			} else if ((j % 4) == 3) {
				umat2->uvalue[i][j] = ((umat->uvalue[i - 1][j - 1] +
																umat->uvalue[i][j - 1] + umat->uvalue[i -
																																			1][j] +
																umat->uvalue[i][j] + umat->uvalue[i][j +
																																		 1]) /
															 ((double) 5.0));
			...
			/*west border */
		{
			if ((j % 4) == 1) {
				...
				umat2->uvalue[i][j] = ((umat->uvalue[i][j - 1] +
																...
																umat->uvalue[i][j] + umat->uvalue[i + 1][j] +
																umat->uvalue[i][j + 1] +
																umat->uvalue[i + 1][j + 1]) / ((double) 6.0));

			} else if ((j % 4) == 3) {
				umat2->uvalue[i][j] = ((umat->uvalue[i][j - 1] +
																umat->uvalue[i][j] + umat->uvalue[i + 1][j] +
																umat->uvalue[i][j + 1] +
																umat->uvalue[i + 1][j + 1]) / ((double) 5.0));
...
/*Corners*/
		umat2->uvalue[0][0] = (umat->uvalue[1][0] + umat->uvalue[0][0] +
													 umat->uvalue[0][1]) / ((double) 3.0);

		umat2->uvalue[(umat->uxdim - 1)][0] =
			(umat->uvalue[(umat->uxdim - 1)][0] +
			 umat->uvalue[(umat->uxdim - 1)][1] +
			 umat->uvalue[(umat->uxdim - 2)][0] +
			 umat->uvalue[(umat->uxdim - 2)][1]) / ((double) 4.0);

		umat2->uvalue[(umat->uxdim - 1)][(umat->uydim - 1)] = (	/* Short cut */
																														umat->
																														uvalue[(umat->
																																		uxdim -
																																		1)][(umat->uydim - 1)] + umat->uvalue[(umat->uxdim - 1)][(umat->uydim - 2)] + umat->uvalue[(umat->uxdim - 2)][(umat->uydim - 1)]) / ((double) 3.0);

		umat2->uvalue[0][(umat->uydim - 1)] = (umat->uvalue[0][(umat->uydim - 1)] + umat->uvalue[1][(umat->uydim - 1)] + umat->uvalue[0][(umat->uydim - 2)]) / ((double) 3.0);	/*Short cut */
	}
...
int
median_umatrix (struct umatrix *umat)
{
	int i, j;
	struct umatrix *umat2;
	...
	if (umat->topol == TOPOL_RECT) {
		...
		umat2->uvalue[0][umat->uydim - 1] =
			median3 (umat->uvalue[1][umat->uydim - 1],
							 umat->uvalue[0][umat->uydim - 1],
							 umat->uvalue[0][umat->uydim - 2]);
		umat2->uvalue[umat->uxdim - 1][umat->uydim - 1] =
			median3 (umat->uvalue[umat->uxdim - 2][umat->uydim - 1],
							 umat->uvalue[umat->uxdim - 1][umat->uydim - 1],
							 umat->uvalue[umat->uxdim - 1][umat->uydim - 2]);
		umat2->uvalue[umat->uxdim - 1][0] =
			median3 (umat->uvalue[umat->uxdim - 2][0],
							 umat->uvalue[umat->uxdim - 1][0],
							 umat->uvalue[umat->uxdim - 1][1]);
		umat2->uvalue[0][0] =
			median3 (umat->uvalue[1][0], umat->uvalue[0][1], umat->uvalue[0][0]);
...
/*Corners*/
		...
		umat2->uvalue[(umat->uxdim - 1)][0] =
			median4 (umat->uvalue[(umat->uxdim - 1)][0],
							 umat->uvalue[(umat->uxdim - 1)][1],
							 umat->uvalue[(umat->uxdim - 2)][0],
		...
		umat2->uvalue[(umat->uxdim - 1)][(umat->uydim - 1)] = median3 (	/* Short cut */
																																		umat->
																																		uvalue[(umat->uxdim - 1)][(umat->uydim - 1)], umat->uvalue[(umat->uxdim - 1)][(umat->uydim - 2)], umat->uvalue[(umat->uxdim - 2)][(umat->uydim - 1)]);


		umat2->uvalue[0][(umat->uydim - 1)] =
			median3 (umat->uvalue[0][(umat->uydim - 1)],
...
void
datamatrix (char *data_file, Map map, Mat mat, char *outputfile, int xswap,
						int yswap)
{
	FILE *in, *out;
	...
	data = (double *) malloc (datadim * sizeof (double));
