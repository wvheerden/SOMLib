...
struct entries *
compute_visual_data (struct teach_params *teach, char *out_file_name)
{
	long length_known;
	...
	datatmp = rewind_entries (data_training_error, &p);

	if ((length_known = data_training_error->flags.totlen_known))
		nod = data_training_error->num_entries;
	else
		nod = 0;
