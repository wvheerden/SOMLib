...
class SOMMap {
	...
	public:
		...
		SOMMap(unsigned int, unsigned int, unsigned int, double***, double);
		~SOMMap();
		...
		double getPrunedNeuronPercentage() const;
		...
		// utility functions
		void markInterpolatingUnits(double);
		void clusterMap();
...
std::ostream& outputDistanceMatrix(std::ostream& out) const;
std::ostream& outputSimilarityMeasureMatrix(std::ostream& out) const;
		// data members
		unsigned int width;
		unsigned int height;
		...
		unsigned int prunedNeuronCount;
