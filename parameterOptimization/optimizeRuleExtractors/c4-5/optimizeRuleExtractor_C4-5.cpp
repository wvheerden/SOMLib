...
class SimulationResult {
	...
	public:
		...
		{
			if ((minOutcomes == source.minOutcomes) && (strcmp(heuristic, source.heuristic) == 0) &&
		...
		void printGeneralizationErrorPlotData(ostream& os, unsigned int totalRecordCount, unsigned int recordIndex)
		{
			...
			os << "0 " << scale(minOutcomes, SimulationResult::minMinOutcomes, SimulationResult::maxMinOutcomes, 0.0, 1.0) << endl;
			...
			os << "2 " << scale(pruningConfidence, SimulationResult::minPruningConfidence, SimulationResult::maxPruningConfidence, 0.0, 1.0) << endl;
			os << "3 " << scale(fisherConfidence, SimulationResult::minFisherConfidence, SimulationResult::maxFisherConfidence, 0.0, 1.0) << endl;
			os << "4 " << scale(redundancy, SimulationResult::minRedundancy, SimulationResult::maxRedundancy, 0.0, 1.0) << endl;
			os << "5 " << scale(meanGeneralizationError, SimulationResult::minMeanGeneralizationError, SimulationResult::maxMeanGeneralizationError, 0.0, 1.0) << endl;
			os << "6 " << scale(meanRuleSetConditionCount, SimulationResult::minMeanRuleSetConditionCount, SimulationResult::maxMeanRuleSetConditionCount, 0.0, 1.0) << endl;
		...
		void printUnscaledGeneralizationErrorPlotData(ostream& os, unsigned int totalRecordCount, unsigned int recordIndex)
		{
			...
			os << "2 " << pruningConfidence << endl;
			os << "3 " << fisherConfidence << endl;
			os << "4 " << redundancy << endl;
			os << "5 " << meanGeneralizationError << endl;
			os << "6 " << meanRuleSetConditionCount << endl;
		...
		{
			os << "minOutcomes\t" << "heuristic\t" << "pruningConfidence\t" << "fisherConfidence\t" << "redundancy\t"
			...
			os << setprecision(14) << fixed << minOutcomes << "\t" << heuristic
...
void processFileName(char* fileName, vector<SimulationResult>& resultStructure, unsigned int recordNumber)
{
	...
	// truncate the first part of the file name, up to end of _minOut_
	strtok(fileName, "_");
	strtok(NULL, "_");
	strtok(NULL, "_");
...
int main(int argc, char**argv)
{
	...
	if (argc != 4)
	{
		...
		cout << "./optimizeLabelClassification errorSummaryFileName minOutMin minOutMax" << endl;
	}
	else
	{
		...
		SimulationResult::minMinOutcomes = atof(argv[2]);
		SimulationResult::maxMinOutcomes = atof(argv[3]);
