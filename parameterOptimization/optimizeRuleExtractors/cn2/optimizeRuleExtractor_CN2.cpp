...
class SimulationResult {
	...
	public:
		...
		SimulationResult()
		{
			...
			errorEstimate = new char[10];
			strcpy(errorEstimate, "");
			starSize = 0.0;
			threshold = 0.0;
		...
		SimulationResult(const SimulationResult& source)
		{
			...
			errorEstimate = new char[10];
			strcpy(errorEstimate, source.errorEstimate);
		...
		{
			if ((strcmp(errorEstimate, source.errorEstimate) == 0) && (starSize == source.starSize) && (threshold == source.threshold) &&
		...
		void setErrorEstimate(char* _errorEstimate)
		{
			strcpy(errorEstimate, _errorEstimate);
		}
		
		void setStarSize(double _starSize)
		{
			starSize = _starSize;
		}
		...
		void printGeneralizationErrorPlotData(ostream& os, unsigned int totalRecordCount, unsigned int recordIndex)
		{
			...
			if (strcmp(errorEstimate, "naive") == 0)
			{
				os << "0 0" << endl;
			} else
			{
			...
			os << "1 " << scale(starSize, SimulationResult::minStarSize, SimulationResult::maxStarSize, 0.0, 1.0) << endl;
			os << "2 " << scale(threshold, SimulationResult::minThreshold, SimulationResult::maxThreshold, 0.0, 1.0) << endl;
			os << "3 " << scale(meanGeneralizationError, SimulationResult::minMeanGeneralizationError, SimulationResult::maxMeanGeneralizationError, 0.0, 1.0) << endl;
			os << "4 " << scale(meanRuleSetConditionCount, SimulationResult::minMeanRuleSetConditionCount, SimulationResult::maxMeanRuleSetConditionCount, 0.0, 1.0) << endl;
		...
		void printUnscaledGeneralizationErrorPlotData(ostream& os, unsigned int totalRecordCount, unsigned int recordIndex)
		{
			...
			os << "1 " << starSize << endl;
			os << "2 " << threshold << endl;
			os << "3 " << meanGeneralizationError << endl;
			os << "4 " << meanRuleSetConditionCount << endl;
		...
		{
			os << "errorEstimate\t" << "starSize\t" << "threshold\t"
			...
			os << setprecision(14) << fixed << errorEstimate << "\t" << starSize << "\t" << threshold
			   << "\t" << meanTrainingError << "\t" << standardDeviationTrainingError
			   << "\t" << meanTrainingMisclassifiedError << "\t" << standardDeviationTrainingMisclassifiedError
			   << "\t" << meanTrainingUnclassifiedError << "\t" << standardDeviationTrainingUnclassifiedError
			   << "\t" << meanGeneralizationError << "\t" << standardDeviationGeneralizationError
	...
	public:
		...
		double starSize;
		static double maxStarSize;
		static double minStarSize;
		
		double threshold;
		static double maxThreshold;
		static double minThreshold;
...
double SimulationResult::maxStarSize = 200.0;
double SimulationResult::minStarSize = 1.0;
double SimulationResult::maxThreshold = 200.0;
double SimulationResult::minThreshold = 0.0;
...
void processFileName(char* fileName, vector<SimulationResult>& resultStructure, unsigned int recordNumber)
{
	...
	// truncate the first part of the file name, up to end of _dataset_
	strtok(fileName, "_");
	strtok(NULL, "_");
	
	// extract error estimate
	char* errorEstimateString = strtok(NULL, "_");
	newResult.setErrorEstimate(errorEstimateString);
	
	// remove _star_
	strtok(NULL, "_");
	
	// extract star size
	char* starSizeString = strtok(NULL, "_");
	double newStarSize = strtod(starSizeString, NULL);
	newResult.setStarSize(newStarSize);
	
	// remove _thr_
	strtok(NULL, "_");
	
	// extract threshold
	char* thresholdString = strtok(NULL, "_");
	double newThreshold = strtod(thresholdString, NULL);;
	newResult.setThreshold(newThreshold);
...
int main(int argc, char**argv)
{
	...
	if (argc != 2)
	{
		...
		cout << "./optimizeRuleExtractor_CN2 errorSummaryFileName" << endl;
	}
	else
	{
