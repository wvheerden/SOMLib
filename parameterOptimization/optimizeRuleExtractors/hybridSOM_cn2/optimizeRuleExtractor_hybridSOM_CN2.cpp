	...
	public:
		...
		bool operator==(const SimulationResult& source)
		{
			if ((dimension == source.dimension) && (learningRate == source.learningRate) && (learningRateDecay == source.learningRateDecay) &&
			    ...
			    (strcmp(errorEstimate, source.errorEstimate) == 0) && (starSize == source.starSize) && (threshold == source.threshold) &&
		...
		void printGeneralizationErrorPlotData(ostream& os, unsigned int totalRecordCount, unsigned int recordIndex)
		{
			...
			os << "6 " << scale(starSize, SimulationResult::minStarSize, SimulationResult::maxStarSize, 0.0, 1.0) << endl;
			os << "7 " << scale(threshold, SimulationResult::minThreshold, SimulationResult::maxThreshold, 0.0, 1.0) << endl;
			os << "8 " << scale(meanGeneralizationError, SimulationResult::minMeanGeneralizationError, SimulationResult::maxMeanGeneralizationError, 0.0, 1.0) << endl;
			os << "9 " << scale(meanRuleSetConditionCount, SimulationResult::minMeanRuleSetConditionCount, SimulationResult::maxMeanRuleSetConditionCount, 0.0, 1.0) << endl;
		...
		void printUnscaledGeneralizationErrorPlotData(ostream& os, unsigned int totalRecordCount, unsigned int recordIndex)
		{
			...
			os << "6 " << starSize << endl;
			os << "7 " << threshold << endl;
			os << "8 " << meanGeneralizationError << endl;
			os << "9 " << meanRuleSetConditionCount << endl;
		...
		void printAll(ostream& os)
		{
			os << "dimension\t" << "learningRate\t" << "learningRateDecay\t" << "kernelWidth\t" << "kernelWidthDecay\t"
			   << "errorEstimate\t" << "starSize\t" << "threshold\t"
			...
			os << setprecision(14) << fixed << dimension << "\t" << learningRate << "\t" << learningRateDecay
			   << "\t" << kernelWidth << "\t" << kernelWidthDecay << "\t" << errorEstimate << "\t" << starSize << "\t" << threshold
			   << "\t" << meanTrainingError << "\t" << standardDeviationTrainingError
			   << "\t" << meanTrainingMisclassifiedError << "\t" << standardDeviationTrainingMisclassifiedError
			   << "\t" << meanTrainingUnclassifiedError << "\t" << standardDeviationTrainingUnclassifiedError
			   << "\t" << meanGeneralizationError << "\t" << standardDeviationGeneralizationError
...
int main(int argc, char**argv)
{
	...
	if (argc != 6)
	{
		...
		cout << "./optimizeLabelClassification errorSummaryFileName dimMin dimMax kernelWidthMin kernelWidthMax" << endl;
	}
	else
	{
